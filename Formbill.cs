...
namespace SuperMaket_project
{
    ...
    public partial class Formbill : Form
    {
        ...
        public void DisplayData()
        {
            ...
            //SqlDataAdapter sda = new SqlDataAdapter("Select * from  ProductSells");
            //DataTable dt = new DataTable();
            //sda.Fill(dt);
            //dataGridView1.DataSource = dt;
        }
            ...
            else
            {
                ...
                switch (comboBox2.SelectedItem)
                {
                    case "single":
                        ...
                        //cmd.CommandText = "insert into ProductSells (ProdName,comBoProds,ProdBareCod,ProdPrice,ProdQoulty,ProdTotalQlty)  values( ' " + textBoxname.Text + "' ,'" + comboBox1.Text + "' , '" + barecode + "' , '" + price + "' , '" + qty + "','" + totalqlty + /*"','" + dTimeStart + "','" + dTimeEnd + */"') ";
                        //cmd.ExecuteNonQuery();
                        DataTable dt = new DataTable();
                        ...
                        //comboBox2.Items.Add("insert into ProductSells where comBoProds='"+ comboBox1.Text + "'");
                        //sda.Fill(dt);
                        ////dataGridView1.DataSource = dt;
                        combSelectProds.Text = "";
        ...
        private void textBoxCustomer_KeyDown_1(object sender, KeyEventArgs e)
        {
            ...
            {
                combSelectProds.Focus();
                combSelectProds.Select();
                combSelectProds.SelectAll();
            }
        }
        ...
        private void Formbill_Load(object sender, EventArgs e  )
        {
            ...
            SqlDataReader rd;
            con.Open();
            string sql = "select * From SendProducts ,LoginUsers ";
        ...
        private void buttonDelet_Click(object sender, EventArgs e)
        {
            ...
            textBoxAllQty.Text = (Convert.ToInt32(textBoxAllQty.Text) + totalqlty + "");
            DataTable dt = new DataTable();
            SqlDataAdapter sda = con.command("Delete  From ProductSells where ProdName='" + combSelectProds.Text + "'");
            sda.Fill(dt);
            dataGridView1.DataSource = dt;
            //cmd.CommandText = "Delete  From ProductSells where prodName='" + textBoxname.Text + "'";
            //cmd.ExecuteNonQuery();
            combSelectProds.Text = "";
            comboBox2.Text = "";
            price = 0;
        ...
        private void buttonNew_Click(object sender, EventArgs e)
        {
            ...
            //textBoxAllQty.Text = (Convert.ToInt32(textBoxAllQty.Text) + totalqlty + "");
            DataTable dt = new DataTable();
             SqlDataAdapter sda = con.command( "Update ProductSells set prodName ='" + combSelectProds.Text + "' where prodName ='" + textSearch.Text + "'");
            //sda.Fill(dt);
            //dataGridView1.DataSource = dt;
            //cmd.ExecuteNonQuery();
            //textBoxname.Text = "";
        ...
        private void combSelectProds_SelectedIndexChanged(object sender, EventArgs e)
        {
            //DataTable dt = new DataTable();
            //SqlDataAdapter sda = con.command("Select *from  SendProducts  where prodName ='" + combSelectProds.Text + "'");
            SqlConnection con = new SqlConnection(@"Data Source = (LocalDB)\MSSQLLocalDB;AttachDbFilename=E:\الجامعة\مستوى ثالث\projects c# forms\SuperMaket_project\DBSPMKT.mdf;Integrated Security = True;");
            con.Open();

         SqlCommand   cmd = new SqlCommand("Select *from  SendProducts  where ProdName ='" + combSelectProds.Text + "'",con);
            
            rd = cmd.ExecuteReader();
            while (rd.Read())
            {
                string priceprod = rd["ProdPriceSell"].ToString();
                string barcodprod = rd["ProdBareCod"].ToString();

                textBoxPrice.Text = priceprod;
                textBoxBCode.Text = barcodprod;
            }
            con.Close();
        }

        private void comEmployees_SelectedIndexChanged(object sender, EventArgs e)
        {
            SqlConnection con = new SqlConnection(@"Data Source = (LocalDB)\MSSQLLocalDB;AttachDbFilename=E:\الجامعة\مستوى ثالث\projects c# forms\SuperMaket_project\DBSPMKT.mdf;Integrated Security = True;");
            con.Open();

            SqlCommand cmd = new SqlCommand("Select *from  LoginUsers  where UserName ='" + comEmployees.Text + "'", con);
            con.Close();
        }
    }
}
